...
const Loading = ({ imageSource, title, supTitle, style }) => {
  const { t } = useTranslation();
  const initLoading = [true, false, false, false, false, false];
  const [loadingArray, setLoadingArray] = useState(initLoading);
  useEffect(() => {
    const interval = setInterval(() => {
      let currentIndex = loadingArray.findIndex(element => element === true);
      if (currentIndex === initLoading.length - 1) {
  ...
  return (
    <View
      ...
      <Image source={imageSource} style={{ marginBottom: 56 }} />
      <View
        style={{
          marginBottom: 24,
          flexDirection: 'row',
        ...
        {loadingArray.map((item, index) => {
          return (
            <React.Fragment key={`loading-${index}`}>
              <View style={item ? styles.big : styles.small} />
            </React.Fragment>
          );
        })}
      </View>
      <Text style={styles.textStyle}>{t(title)}</Text>
      <Text style={styles.textStyle}>{t(supTitle)}</Text>
    </View>
  );
};
